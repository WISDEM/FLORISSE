...
if __name__ == "__main__":
    ...
    #np.savetxt('XY.txt',np.c_[prob['turbineX'],prob['turbineY'],prob['turbineZ']])
    ...
    for i in range(0, nTurbsH1):
        plt.plot(prob['turbineX'][i],prob['turbineY'][i], 'ok', label='Short Optimized')
    plt.plot(turbineX2, turbineY2, '.g', label='Tall Original')
    for i in range(nTurbsH1, nTurbsH1+nTurbsH2):
        plt.plot(prob['turbineX'][i],prob['turbineY'][i], 'og', label='Tall Optimized')
    ...
    prob.driver = pyOptSparseDriver()
    prob.driver.options['optimizer'] = 'SNOPT'
    prob.driver.add_objective('maxAEP', scaler=1E-2)
