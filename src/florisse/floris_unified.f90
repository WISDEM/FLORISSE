...
!                ct:out a_in:out
SUBROUTINE FLORIS_UNIFIED_BV(nturbines, kd, initialwakedisplacement, &
...
& cos_spread, a_in, a_inb, axialindprovided, useaubu, vinf, kecorrarray&
& , mu, au, bu, velocitiesturbinesb, nbdirs)
!  Hint: ndirs should be the maximum number of differentiation directions
  IMPLICIT NONE
! define precision to be the standard for a double precision ! on local system
  INTEGER, PARAMETER :: dp=KIND(0.d0)
! in
...
& turbinexw, turbineyw
  REAL(dp), DIMENSION(ndirs, nturbines), intent(out) :: yaw_degb, ctb, a_inb, &
& turbinexwb, turbineywb
  REAL(dp), DIMENSION(nturbines), INTENT(IN) :: rotordiameter
  REAL(dp), DIMENSION(ndirs, nturbines), intent(out) :: rotordiameterb
  REAL(dp), DIMENSION(3), INTENT(IN) :: me, mu
  REAL(dp), INTENT(IN) :: au, bu
...
! local (General)
  REAL(dp), DIMENSION(nturbines) :: ke, yaw
  REAL(dp), DIMENSION(ndirs, nturbines) :: keb, yawb
  REAL(dp) :: deltax
  REAL(dp), DIMENSION(ndirs) :: deltaxb
  INTEGER :: turb, turbi, zone
  REAL(dp), PARAMETER :: pi=3.141592653589793_dp
! local (Wake centers and diameters)
! in rotor diameters    
  ...
  REAL(dp) :: wakeangleinit, zeroloc
  REAL(dp), DIMENSION(ndirs) :: wakeangleinitb, zerolocb
  REAL(dp) :: factor, displacement, x, x1, x2, y1, y2, dy1, dy2
  REAL(dp), DIMENSION(ndirs) :: factorb, displacementb, xb, x1b, x2b&
& , y1b, dy1b
  REAL(dp) :: wakediameter0
  REAL(dp), DIMENSION(ndirs) :: wakediameter0b
  REAL(dp), DIMENSION(nturbines, nturbines, 3) :: wakeoverlaptrel_mat, &
& wakediameterst_mat
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines, 3) :: &
& wakeoverlaptrel_matb, wakediameterst_matb
  REAL(dp), DIMENSION(nturbines, nturbines, 3) :: wakediameterst_mat
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines, 3) :: &
& wakediameterst_matb
  REAL(dp), DIMENSION(nturbines, nturbines) :: wakecentersyt_mat
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines) :: &
& wakecentersyt_matb
  REAL(dp), DIMENSION(nturbines, nturbines) :: wakecentersyt_mat
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines) :: &
& wakecentersyt_matb
! local (Wake overlap)
  REAL(dp) :: rmax
  REAL(dp), DIMENSION(ndirs) :: rmaxb
  REAL(dp), DIMENSION(nturbines, nturbines, 3) :: cosfac_mat
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines, 3) :: cosfac_matb
! local (Velocity)
  REAL(dp), DIMENSION(nturbines) :: a, kearray
  REAL(dp), DIMENSION(ndirs, nturbines) :: ab, kearrayb
  REAL(dp), DIMENSION(3) :: mmu
  REAL(dp), DIMENSION(ndirs, 3) :: mmub
  REAL(dp) :: s, wakeeffcoeff, wakeeffcoeffperzone
  REAL(dp), DIMENSION(ndirs) :: sb, wakeeffcoeffb, &
& wakeeffcoeffperzoneb
! out
  REAL(dp), DIMENSION(nturbines) :: velocitiesturbines
  REAL(dp), DIMENSION(ndirs, nturbines) :: velocitiesturbinesb
  INTRINSIC COS
  INTRINSIC KIND
  INTRINSIC SIN
  INTRINSIC DABS
  ...
  REAL(dp) :: temp1
  REAL(dp) :: temp0
  REAL(dp) :: tempb9(ndirs)
  REAL(dp) :: tempb8(ndirs)
  REAL :: tempb7(ndirs)
  REAL(dp) :: tempb6(ndirs)
  REAL(dp) :: tempb5(ndirs)
  REAL(dp) :: tempb4(ndirs)
  REAL(dp) :: tempb3(ndirs)
  REAL(dp) :: tempb2(ndirs)
  REAL(dp) :: tempb1(ndirs)
  REAL(dp) :: tempb0(ndirs)
  REAL(dp) :: tempb13(ndirs)
  REAL(dp) :: tempb12(ndirs)
  REAL(dp) :: tempb11(ndirs)
  REAL(dp) :: tempb10(ndirs)
  DOUBLE PRECISION :: dabs0b(ndirs)
  REAL(dp) :: temp11
  REAL(dp) :: temp10
  REAL(dp) :: tempb(ndirs)
...
&         ) THEN
          ...
          ELSE
            ...
            CALL PUSHCONTROL1B(1)
          END IF
          cosfac_mat(turbi, turb, zone) = 0.5_dp*(1.0_dp+COS(pi*dabs0/&
&           rmax))
        END DO
        CALL PUSHCONTROL1B(1)
      ELSE
      ...
      ELSE
        ...
        DO zone=3,1,-1
          ...
          IF (branch .EQ. 0) THEN
            ...
            temp10 = rotordiameter(turb)**2*wakeoverlaptrel_mat(turbi, &
&             turb, zone)
            temp9 = cosfac_mat(turbi, turb, zone)**2
            DO nd=1,nbdirs
              tempb11(nd) = wakeeffcoeffperzoneb(nd)/temp8**2
              tempb12(nd) = -(temp9*temp10*2*tempb11(nd)/temp8)
              cosfac_matb(nd, turbi, turb, zone) = cosfac_matb(nd, turbi&
&               , turb, zone) + temp10*2*cosfac_mat(turbi, turb, zone)*&
&               tempb11(nd)
              rotordiameterb(nd, turb) = rotordiameterb(nd, turb) + &
&               tempb12(nd) + wakeoverlaptrel_mat(turbi, turb, zone)*&
&               temp9*2*rotordiameter(turb)*tempb11(nd)
              wakeoverlaptrel_matb(nd, turbi, turb, zone) = &
&               wakeoverlaptrel_matb(nd, turbi, turb, zone) + temp9*&
&               rotordiameter(turb)**2*tempb11(nd)
...
&             mmu(zone)
            temp7 = rotordiameter(turb)**2*wakeoverlaptrel_mat(turbi, &
&             turb, zone)
            temp6 = cosfac_mat(turbi, turb, zone)**2
            DO nd=1,nbdirs
              tempb8(nd) = wakeeffcoeffperzoneb(nd)/temp5**2
              tempb9(nd) = -(temp6*temp7*2*tempb8(nd)/temp5)
              tempb10(nd) = 2.0_dp*mmu(zone)*tempb9(nd)
              cosfac_matb(nd, turbi, turb, zone) = cosfac_matb(nd, turbi&
&               , turb, zone) + temp7*2*cosfac_mat(turbi, turb, zone)*&
&               tempb8(nd)
              rotordiameterb(nd, turb) = rotordiameterb(nd, turb) + &
&               tempb9(nd) + wakeoverlaptrel_mat(turbi, turb, zone)*&
&               temp6*2*rotordiameter(turb)*tempb8(nd)
              wakeoverlaptrel_matb(nd, turbi, turb, zone) = &
&               wakeoverlaptrel_matb(nd, turbi, turb, zone) + temp6*&
&               rotordiameter(turb)**2*tempb8(nd)
...
&           rotordiameter(turbi))
          DO nd=1,nbdirs
            tempb7(nd) = -(pi*SIN(pi*(dabs0/rmax))*0.5_dp*cosfac_matb(nd&
...
! in
  REAL(dp), INTENT(IN) :: x, x0, x1, y0, dy0, y1, dy1
  REAL(dp), DIMENSION(ndirs) :: xb, x0b, x1b, y0b, dy0b
! out
!, dy_dx
  REAL(dp) :: y
  REAL(dp), DIMENSION(ndirs) :: yb
! local
  REAL(dp) :: c3, c2, c1, c0
  REAL(dp), DIMENSION(ndirs) :: c3b, c2b, c1b, c0b
  INTRINSIC KIND
  INTEGER :: nd
  INTEGER :: nbdirs
  REAL(dp) :: temp3
  ...
  REAL(dp) :: tempb9(ndirs)
  REAL(dp) :: tempb8(ndirs)
  REAL(dp) :: tempb7(ndirs)
  REAL(dp) :: tempb6(ndirs)
  REAL(dp) :: tempb5(ndirs)
  REAL(dp) :: tempb4(ndirs)
  REAL(dp) :: tempb19(ndirs)
  REAL(dp) :: tempb3(ndirs)
  REAL(dp) :: tempb18(ndirs)
  REAL(dp) :: tempb2(ndirs)
  REAL(dp) :: tempb17(ndirs)
  REAL(dp) :: tempb1(ndirs)
  REAL(dp) :: tempb16(ndirs)
  REAL(dp) :: tempb0(ndirs)
  REAL(dp) :: tempb15(ndirs)
  REAL(dp) :: tempb14(ndirs)
  REAL(dp) :: tempb13(ndirs)
  REAL(dp) :: tempb12(ndirs)
  REAL(dp) :: tempb11(ndirs)
  REAL(dp) :: tempb10(ndirs)
  ...
  REAL(dp) :: temp10
  REAL(dp) :: tempb(ndirs)
  REAL(dp) :: tempb30(ndirs)
  REAL(dp) :: tempb29(ndirs)
  REAL(dp) :: tempb28(ndirs)
  REAL(dp) :: tempb27(ndirs)
  REAL(dp) :: tempb26(ndirs)
  REAL(dp) :: tempb25(ndirs)
  REAL(dp) :: temp
  REAL(dp) :: tempb24(ndirs)
  REAL(dp) :: tempb23(ndirs)
  REAL(dp) :: tempb22(ndirs)
  REAL(dp) :: temp9
  REAL(dp) :: tempb21(ndirs)
  REAL(dp) :: temp8
  REAL(dp) :: tempb20(ndirs)
  REAL(dp) :: temp7
  REAL(dp) :: temp6
  REAL(dp) :: temp5
  REAL(dp) :: temp4
...
& rotordiameter
  REAL(dp), DIMENSION(ndirs, nturbines) :: turbineyb, rotordiameterb
  REAL(dp), DIMENSION(nturbines, nturbines, 3), INTENT(IN) :: &
& wakediameters
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines, 3) :: &
& wakediametersb
  REAL(dp), DIMENSION(nturbines, nturbines), INTENT(IN) :: wakecenters
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines) :: wakecentersb
! out    
  REAL(dp), DIMENSION(nturbines, nturbines, 3) :: wakeoverlaptrel_mat
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines, 3) :: &
...
! local
  ...
  REAL(dp), PARAMETER :: pi=3.141592653589793_dp, tol=0.000001_dp
  REAL(dp) :: ovdyd, ovr, ovrr, ovl, ovz
  REAL(dp), DIMENSION(ndirs) :: ovdydb, ovrb, ovrrb, ovlb, ovzb
  REAL(dp), DIMENSION(nturbines, nturbines, 3) :: wakeoverlap
  REAL(dp), DIMENSION(ndirs, nturbines, nturbines, 3) :: &
& wakeoverlapb
  INTRINSIC KIND
  ...
  REAL(dp) :: temp1
  REAL(dp) :: temp0
  REAL(dp) :: tempb2(ndirs, nturbines, 3)
  REAL(dp) :: tempb1(ndirs)
  REAL(dp) :: tempb0(ndirs)
  REAL(dp) :: tempb(ndirs)
  REAL(dp) :: temp
...
! in
  INTEGER, INTENT(IN) :: nturbines
  REAL(dp), DIMENSION(nturbines), INTENT(IN) :: ct
  REAL(dp), DIMENSION(ndirs, nturbines) :: ctb
! local
  INTEGER :: i
! out
  REAL(dp), DIMENSION(nturbines) :: axial_induction
  REAL(dp), DIMENSION(ndirs, nturbines) :: axial_inductionb
  INTRINSIC KIND
  INTRINSIC SQRT
  INTEGER :: nd
  INTEGER :: branch
