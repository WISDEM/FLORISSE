...
import numpy as np
from EEOver.skewedToRotatedEllipse import skewedToRotatedEllipse
from EEOver.ellipseEllipseOverlap import ellipse_ellipse_overlap
...
class floris_windframe(Component):
    ...
    def __init__(self, nTurbines, nSamples=0):
        ...
        self.add('turbineHubZ', Array(np.zeros(nTurbines), iotype='out', units='m', \
                                    desc='z coordinates of turbine hubs'))
...
class floris_wcent_wdiam(Component):
    ...
    def __init__(self, nTurbines, nSamples=0):
        ...
        self.add('wakeCentersZT', Array(np.zeros(nTurbines*nTurbines), iotype='out', dtype='float', \
                                        desc='wake center z position at each turbine'))
        self.add('wakeSkewT', Array(np.zeros(nTurbines*nTurbines), iotype='out', dtype='float', \
                                        desc='wake skew at each turbine')) 
        ...
        self.add('wakeWidthsT', Array(np.zeros(nTurbines*nTurbines*3), iotype='out', dtype='float', \
                                         desc='wake width of each zone of each wake at each turbine'))
        ...
        self.add('wakeWidths', Array(np.zeros([nSamples, nTurbines, 3]), iotype='out', dtype='float', desc='wake width of each zone of each wake at each turbine'))
        ...
        self.add('wakeSkewS', Array(np.zeros([nSamples, nTurbines]), iotype='out', units='m', desc='skew of wakes at measurement points'))
    ...
    def execute(self):
        ...
        wakeSkewS = np.zeros((nSamples, nTurbines))
        wakeCentersYT_mat = np.zeros((nTurbines, nTurbines))
        wakeCentersZT_mat = np.zeros((nTurbines, nTurbines))
        wakeSkewT_mat = np.zeros((nTurbines, nTurbines))
        for turb in range(0, nTurbines):
            ...
            for loc in range(0, nSamples):  # at velX-locations
                ...
                wakeSkewS[loc, turb] = wakeSkew[turb]
            ...
            for turbI in range(0, nTurbines):  # at turbineX-locations
                ...
                wakeCentersZT_mat[turbI, turb] = hubHeight[turb]
                wakeSkewT_mat[turbI, turb] = wakeSkew[turb] + deltax * wakeSkewCoefficient
        ...
        wakeWidths = np.zeros((nSamples,nTurbines, 3))
        wakeDiametersT_mat = np.zeros((nTurbines, nTurbines, 3))
        wakeWidthsT_mat = np.zeros((nTurbines, nTurbines, 3))
        for turb in range(0, nTurbines):
            wakeDiameter0 = rotorDiameter[turb]
            for loc in range(0, nSamples):  # at velX-locations
                deltax = velX[loc]-turbineXw[turb]
                for zone in range(0,3):
                    wakeDiameters[loc, turb, zone] = np.maximum(wakeDiameter0 + 2*ke[turb]*me[zone]*np.maximum(deltax, 0),0)
                    if adjustInitialWakeDiamToYaw:
                        wakeWidths[loc, turb, zone] = wakeDiameters[loc, turb, zone] * np.cos(yaw[turb])
                    else:  
                        wakeWidths[loc, turb, zone] = wakeDiameters[loc, turb, zone]
            for turbI in range(0, nTurbines):  # at turbineX-locations
                ...
                for zone in range(0, 3):
                    ...
                    if adjustInitialWakeDiamToYaw:
                        wakeWidthsT_mat[turbI, turb, zone] = wakeDiametersT_mat[turbI, turb, zone] * np.cos(yaw[turb])
                    else:
                        wakeWidthsT_mat[turbI, turb, zone] = wakeDiametersT_mat[turbI, turb, zone]

        wakeDiametersT_vec = np.zeros(3*nTurbines*nTurbines)
        wakeWidthsT_vec = np.zeros(3*nTurbines*nTurbines)
        ...
        for i in range(0, nTurbines):
            wakeWidthsT_vec[(3*nTurbines*i):(3*nTurbines*i+nTurbines)] = wakeWidthsT_mat[i, :, 0]
            wakeWidthsT_vec[3*nTurbines*i+nTurbines:3*nTurbines*i+2*nTurbines] = wakeWidthsT_mat[i, :, 1]
            wakeWidthsT_vec[3*nTurbines*i+2*nTurbines:3*nTurbines*i+3*nTurbines] = wakeWidthsT_mat[i, :, 2]

        wakeCentersYT_vec = np.zeros(nTurbines*nTurbines)
        wakeCentersZT_vec = np.zeros(nTurbines*nTurbines)
        wakeSkewT_vec = np.zeros(nTurbines*nTurbines)
        for i in range(0, nTurbines):
            wakeCentersYT_vec[(nTurbines*i):(nTurbines*i+nTurbines)] = wakeCentersYT_mat[i, :]
            wakeCentersZT_vec[(nTurbines*i):(nTurbines*i+nTurbines)] = wakeCentersZT_mat[i, :]
            wakeSkewT_vec[(nTurbines*i):(nTurbines*i+nTurbines)] = wakeSkewT_mat[i, :]

        self.wakeCentersYT = wakeCentersYT_vec
        self.wakeCentersZT = wakeCentersZT_vec
        self.wakeDiametersT = wakeDiametersT_vec
        self.wakeWidthsT = wakeWidthsT_vec
        self.wakeSkewT = wakeSkewT_vec
        self.wakeDiameters = wakeDiameters
        self.wakeWidths = wakeWidths
        self.wakeCentersY = wakeCentersY
        self.wakeCentersZ = wakeCentersZ
        self.wakeSkewS = wakeSkewS
...
class floris_overlap(Component):
    ...
    def __init__(self, nTurbines):
        ...
        self.add('turbineHubZ', Array(np.zeros(nTurbines), dtype='float', iotype='in', units='m', \
                desc='Z positions of hubs of all turbines'))
        ...
        self.add('wakeCentersZT', Array(np.zeros(nTurbines*nTurbines), iotype='in', units='m', \
                                              desc='Z positions of all wakes at each turbine'))
        self.add('wakeSkewT', Array(np.zeros(nTurbines*nTurbines), iotype='in', units='m', \
                                              desc='Skew of all wakes at each turbine'))       
        self.add('wakeDiametersT', Array(np.zeros(nTurbines*nTurbines*3), iotype='in', units='m',\
                                               desc='diameters of all turbines wake zones'))
        self.add('wakeWidthsT', Array(np.zeros(nTurbines*nTurbines*3), iotype='in', units='m',\
                                               desc='widths of all turbines wake zones'))
        # Explicitly size output arrays
    ...
    def execute(self):
        ...
        # convert the input vector to the array used for calculations
        for i in range(0, nTurbines):
                wakeWidthsT_mat[i, :, 0] = self.wakeWidthsT[3*nTurbines*i:3*nTurbines*i+nTurbines]
                wakeWidthsT_mat[i, :, 1] = self.wakeWidthsT[3*nTurbines*i+nTurbines:3*nTurbines*i+2*nTurbines]
                wakeWidthsT_mat[i, :, 2] = self.wakeWidthsT[3*nTurbines*i+2*nTurbines:3*nTurbines*i+3*nTurbines]

        # convert the input vector to the array used for calculations
        for i in range(0, nTurbines):
                wakeCentersYT_mat[i, :] = self.wakeCentersYT[nTurbines*i:nTurbines*i+nTurbines]
                wakeCentersZT_mat[i, :] = self.wakeCentersZT[nTurbines*i:nTurbines*i+nTurbines]
                wakeSkewT_mat[i, :] = self.wakeSkewT[nTurbines*i:nTurbines*i+nTurbines]
        ...
        wakeOverlapT = calcOverlapAreas(self.turbineXw, self.turbineYw, self.turbineHubZ, self.rotorDiameter, wakeDiametersT_mat, wakeWidthsT_mat, wakeCentersYT_mat, wakeCentersZT_mat, wakeSkewT_mat)
...
class floris_power(Component):
    ...
    def __init__(self, nTurbines, nSamples=0, shearProfileSize=0):
        ...
        self.add('wakeWidths', Array(np.zeros([nSamples, nTurbines, 3]), iotype='in', units='m', desc='width of wake zones at measurement points'))
        ...
        self.add('wakeSkewS', Array(np.zeros([nSamples, nTurbines]), iotype='in', desc='skew of wakes at measurement points'))
    ...
    def execute(self):
        ...
        ke = self.parameters.ke
        k_nearwake = self.parameters.k_nearwake
        ...
        usePredefinedShearProfile = self.parameters.usePredefinedShearProfile
        shearZh = self.parameters.shearZh
        if usePredefinedShearProfile and shearProfileSize>0:
            shearProfileZnorm = self.shearProfileZnorm
            shearProfileUnorm = self.shearProfileUnorm
        else:
        ...
        wakeSkewS = self.wakeSkewS
        wakeDiameters = self.wakeDiameters
        wakeWidths = self.wakeWidths
        ...
        if usePredefinedShearProfile and shearProfileSize>0:
            ...
            fShear = interp1d(shearProfileZnorm, shearProfileUnorm)
            Znorm = np.maximum(np.minimum(velZ/shearZh,np.max(shearProfileZnorm)),np.min(shearProfileZnorm)) # normalize z-positions, and bound within predefined shear profile (prevents extrapolation)
            self.ws_array = self.ws_array*fShear(Znorm)
        else:
        ...
        for turb in range(0, nTurbines):
            ...
            hs_axind = rotorDiameter[turb] # height of axial induction zone is assumed to be equal to rotor diameter
            ws_axind = hs_axind * np.cos(yaw[turb]) # width is adjusted to yaw

            for loc in range(0, nSamples):
                deltax = velX[loc] - turbineXw[turb]
                deltay = velY[loc] - wakeCentersY[loc, turb]
                deltaz = velZ[loc] - wakeCentersZ[loc, turb]
                c = wakeSkewS[loc,turb] + deltax * wakeSkewCoefficient
                
                axialIndAndNearRotor = 2*axialInd[turb]*(0.5+np.arctan(2.0*k_nearwake*deltax/rotorDiameter[turb])/np.pi)
                
                # check in which zone measurement point is
                inZone = np.zeros(4)
                if deltax > 0:
                    for zone in range(3):
                        hs = wakeDiameters[loc, turb, zone] # wake height (non-skewed)
                        ws = wakeWidths[loc, turb, zone] # wake width (non-skewed)
                        if ws>0 and hs>0:
                            if ((deltay+c*deltaz)/(ws/2))**2+(deltaz/(hs/2))**2 < 1: # check if in skewed wake
                                inZone[zone] = 1
                elif deltax <= 0: # zone 4 is axial induction zone in front of rotor (assumed non-skewed!)
                    if (deltay/(ws_axind/2))**2+(deltaz/(hs_axind/2))**2 < 1:
                        inZone[3] = 1

                # apply velocity reduction factor, depending on zone
                if inZone[0]: # if in zone 1
...
def calcOverlapAreas(turbineX,turbineY,turbineHubZ,rotorDiameter,wakeDiameters,wakeWidths,wakeCentersY,wakeCentersZ,wakeSkew):
    ...
    for turbI in range(0,nTurbines):
        radiusRotor = rotorDiameter[turbI]/2
        rotorY = turbineY[turbI]
        rotorZ = turbineHubZ[turbI]
        for turb in range(0,nTurbines):
            skewWake = wakeSkew[turbI,turb]
            wakeY = wakeCentersY[turbI,turb]
            wakeZ = wakeCentersZ[turbI,turb]
            for zone in range(3):
                if turbineX[turbI] > turbineX[turb]:
                    widthWake = wakeWidths[turbI,turb,zone]
                    heightWake = wakeDiameters[turbI,turb,zone]
                    phi, w, h = skewedToRotatedEllipse(widthWake, heightWake, skewWake) # skewed ellipse is a rotated ellipse with width w, height h and rotation phi
                    wakeOverlap[turbI,turb,zone] = ellipse_ellipse_overlap(w, h, wakeY, wakeZ, phi, radiusRotor, radiusRotor, rotorY, rotorZ, 0.0) # calculate overlap of ellipse with circle

    for turb in range(0,nTurbines):
        for turbI in range(0,nTurbines):
            wakeDiameters[turbI,turb,zone]/2
